    ...
    public class Parameters
    {
        ...
        public void ValidateParameters() 
        {
            foreach (ParameterType parameterType in Enum.GetValues(typeof(ParameterType))) 
            {
                ...
                {
                    throw new ArgumentException("Не хватает одного из параметров: " + parameterType);
                }

                if (ModelParameters[ParameterType.BerthCount].Value *
                    ParametersConstants.BedWidthPerBerth > ModelParameters[ParameterType.MainPartWidth].Value) 
                {
                    throw new ArgumentException("Ширина кровати слишком мала для указанного количества спальных мест");
                }

                if (ModelParameters[ParameterType.PersonsHeight].Value +
                    ParametersConstants.PersonsHeightBedLengthDiff > ModelParameters[ParameterType.MainPartLength].Value) 
                {
                    throw new ArgumentException("Длина кровати слишком мала для указанного роста человека");
                }
            }
        }
